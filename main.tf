...
resource "aws_iam_policy" "vpcAccessExecution" {
  count  = local.create_role && length(coalesce(var.vpc_subnet_ids, [])) >= 1 ? 1 : 0
  name   = "${var.function_name}-vpcAccessExecution-policy"
  policy = join("", data.aws_iam_policy_document.vpcAccessExecution.*.json)
}
...
resource "aws_iam_role_policy_attachment" "vpcAccessExecution" {
  ...
  policy_arn = aws_iam_policy.vpcAccessExecution[0].arn
